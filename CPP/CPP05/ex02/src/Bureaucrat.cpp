...
/*   Updated: 2022/10/03 13:35:50 by estoffel         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../incl/Bureaucrat.hpp"

Bureaucrat::Bureaucrat(void): _Name("Weird Manager"), _Grade(40) {
...
void	Bureaucrat::executeForm(const Form& form) {
	...
	if (_Grade > 150)
		...
		throw Bureaucrat::GradeTooHighException();
	else if (_Grade <= 1 && _Grade > form.getExeGrade())
		std::cout << "Bureaucrat " << _Name << " couldn't execute " << form.getName() << " because his grade is too low" << std::endl;
	else {
		std::cout << "Bureaucrat " << _Name << " executed " << form.getName() << "\n" << std::endl;
		form.execute(*this);
	}
}
